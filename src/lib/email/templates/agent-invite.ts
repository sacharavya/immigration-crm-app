...
export function agentInviteEmail(args: Args): {
  ...
  const subject = `Your genzdatalabs Immigration partner account`;

  const bodyHtml = `<p style="margin:0 0 12px 0;">Hi ${escapeHtml(args.name)},</p>
<p style="margin:0 0 12px 0;">A referral partner account has been created for you on the genzdatalabs Immigration CRM. Sign in with the credentials below — you'll be asked to set a new password on first login. You'll be able to register your clients and track the ones you've referred.</p>
...
<p style="margin:24px 0 0 0;color:#57534e;font-size:14px;">If you weren't expecting this email, please contact your genzdatalabs Immigration representative.</p>`;

  const text = `Hi ${args.name},

A referral partner account has been created for you on the genzdatalabs Immigration CRM.
...
If you weren't expecting this email, please contact your genzdatalabs Immigration representative.
...
genzdatalabs Immigration Consulting Inc.`;
  ...
  return {
    subject,
    html: emailLayout({
      previewText: "Your genzdatalabs Immigration partner account is ready.",
      bodyHtml,
    }),
    text,
  };
